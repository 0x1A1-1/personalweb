...
class Handler
{
	...
	private function buildSiteArray( )
	{
		...
		{
			$this->site = unserialize( file_get_contents( \BASE_SERVICES_DIR . '/' . Configuration::PUBLISHED_DATA_LOCATION ) );
		}
	}
	...
	{
		if ( isset( $_SERVER['REQUEST_SCHEME'] ) === false )
		{
			$_SERVER['REQUEST_SCHEME'] = 'http';
		}

		$this->request = parse_url( $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'] );
		$this->request['directories'] = explode( '/', ltrim( $this->request['path'], '/' ) );
		...
		else
		{
			...
			$this->request['file'] = ltrim( $this->request['path'], '/' );
		}
		...
		if ( strpos( $this->request['file'], '.' ) === false )
		{
			 ...
			 */
			$file = ltrim( rtrim( $this->request['path'], '/' ), '/' ) . '.html';
	...
	{
		if ( $directory === 'store' || $directory === 'blog' || $directory === 'apps'
		|| ( isset( $this->site['dynamic'] ) === true && isset( $this->site['dynamic'][$directory] ) === true ) )
		{
			return true;
		}
